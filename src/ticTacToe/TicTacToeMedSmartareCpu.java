...
public class TicTacToeMedSmartareCpu {
	...
	public static void main(String[] args) {
		...
		while (true) {
			...
			while (true) {
				try {
					...
					while (playerPosition <= 0 || playerPosition > 9) {
						System.out.println("Du kan bara v�lja position med siffrorna 1 - 9, pr�va igen.");
						try {
							playerPosition = scanner.nextInt();
			...
			// Loop som kontrollerar om positionen �r upptagen, om den �r det får man pr�va
			// igen. med exceptionhantering f�r input.
			while (playerPositions.contains(playerPosition) || cpuPositions.contains(playerPosition)) {
				System.out.println("position upptagen, pr�va igen");
				while (true) {
					try {
						Scanner scanner = new Scanner(System.in);
						playerPosition = scanner.nextInt();
			...
			// datorns tur
			// Datorn går igenom alla positioner på spelbrädet. Om positionen är
			// upptagen så prövar den nästa. Om en placering i positionen resulterar i
			// vinst så kommer
			...
			// om ingen av de möjliga positionera resulterar i vinst så slumpar datorn
			// sitt val.
			int cpuPosition = rand.nextInt(9) + 1;
			while (playerPositions.contains(cpuPosition) || cpuPositions.contains(cpuPosition)) { // om den framslumpade
																									// positionen är
																									// upptagen så
																									// slumpas en ny
																									// fram.
	...
	// metod för att kontrollera om någon har vunnit. Skapar en lista för varje
	// positioner som resulterar i en vinst. Placerar sedan alla dom listorna i en
	// stor lista. Sedan loopas denna igenom och
	// jämförs med spelarens lagda positioner. Om spelarens positioner innehåller
	// allt från en av vinstlistorna så printas det ut vem som vann. Om brädet
	// är fullt men ingen har vunnit så
	...
	// metod för att placera spelarens val. Beroende på vems tur det är så
	// placeras rätt symbol ut på den valda positionen på brädet.
